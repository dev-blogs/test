...
/**
   This program demonstrates the transfer of object references
...
{  
   ...
   {  
      ...
      curvePanel.setPreferredSize(new Dimension(WIDTH / 2, 
         HEIGHT));
      shapePanel = new ShapePanel();

      contentPane.add(new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
         curvePanel, shapePanel), BorderLayout.CENTER);
   ...
   */
   private void copy()
   {  
      LocalSelection selection = new LocalSelection(
         curvePanel.getShape());
...
"application/x-java-jvm-local-objectref;class=java.awt.Shape");
         if (contents.isDataFlavorSupported(flavor))
            shapePanel.setShape(
               (Shape)contents.getTransferData(flavor));
      }
   ...
   */   
   ...
   public boolean isDataFlavorSupported(DataFlavor flavor) 
   {
      return "application".equals(flavor.getPrimaryType()) 
         && "x-java-jvm-local-objectref".equals(
            flavor.getSubType()) 
